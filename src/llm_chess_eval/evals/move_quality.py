...
Default config:
    ...
    - QUALITY_DECAY_CONSTANT = 150 (same as PlayStrength)
    - Phase weights 1/1.5/2/3 at ply boundaries 10/20/30 (same as PlayStrength)
...
def _game_phase_weight(ply: int) -> float:
    """Softened phase weight; matches PlayStrength. Endgame at 3× opening."""
    if ply < 10:
        return 1.0
    if ply < 20:
        return 1.5
...
def _move_quality_from_cp_loss(cp_loss: int) -> float:
    """Exponential decay quality; matches PlayStrength."""
