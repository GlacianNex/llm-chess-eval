...
- **PlayQuality** — move strength once a legal move is found. Same per-move quality and phase weighting as Reliability, but without the retry cost (retries don't hurt the score once a legal move is found). Played at Stockfish skill 5 (~1700 ELO, strong amateur) with max 3 retries.
