...
> **Heads up on a counter-intuition before you read the table.** You'll notice that `gemini-3.1-flash-lite` (86.5% first-attempt-legal, 5% forfeit rate) scores *higher* on PlayStrength than `gpt-5` (99.8% first-attempt-legal, 0% forfeit rate). That ordering is intentional, not a metric bug. PlayStrength is a composite that weights move quality and game depth alongside rule-following — and Flash Lite plays substantially better chess (opening ACPL 45 vs GPT-5's 85; reaches mean ply 43 vs GPT-5's 27) while GPT-5 plays disciplined but competent-club-level moves. **In chess terms: Flash Lite is a strong-club player who occasionally needs a do-over; GPT-5 is a perfectly-disciplined player whose moves are mid-range.** The composite reports the stronger chess player, not the more rule-compliant one. The 5% forfeit costs Flash Lite ~0.025 on the composite; GPT-5's move-quality gap costs it ~0.20. Quality + depth dominate by ~8×. If you want a pure rule-following ranking, sort by the first-attempt-legal or forfeit-rate columns directly — they're top-line diagnostics for exactly that. Full decomposition of this paradox is in [The Flash Lite outlier](#the-flash-lite-outlier--why-a-budget-non-reasoning-model-nearly-tops-the-matrix) below.
...
Two scores per model, both bounded `[0, 1]`. **PlayStrength** is the headline composite — it multiplies move quality, retry-cost discipline (`0.25^retries`), and a phase weight that counts late-game plies more, across full games vs Stockfish skill 3 (~1500 ELO amateur). **MoveQuality** is the supplemental — it strips the retry-cost factor and runs against a harder Stockfish (skill 5, ~1700 ELO) so you can isolate "how good are the moves themselves" from "how often does the model find a legal move on first try." Stockfish self-play would score 1.0; the published PlayStrength top is 0.485. Diagnostic columns: **first-legal** = fraction of plies where the model's first proposal was legal (zero retries); **avg retries** = total retries across all plies divided by total plies; **forfeit rate** = fraction of games that ended because the model couldn't produce a legal move within 10 retries. Full formulas in [METHODOLOGY.md](METHODOLOGY.md).
